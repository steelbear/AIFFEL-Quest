...
- [V] 주석을 보고 작성자의 코드가 이해되었나요?
  > 각 코드에 대한 주석이 달려있었고, 실습2에서는 해당 컬럼을 사용한 이유에 대해서도 정리가 되어있었습니다.
- [V] 코드가 에러를 유발할 가능성이 없나요?
  > 다운받은 코드를 실행 했을때 에러가 없었습니다.
- [V] 코드 작성자가 코드를 제대로 이해하고 작성했나요?
  > 파이썬 람다함수사용, 시각화 코드를 보았을때 코드를 이해하고 작성하였다는 생각이 들었습니다.
- [V] 코드가 간결한가요?
  > 의미없는 코드가 없었기에 간결하다 라고 생각했습니다.
